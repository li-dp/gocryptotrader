...
func (y *Yobit) CancelAllOrders(_ exchange.OrderCancellation) (exchange.CancelAllOrdersResponse, error) {
	...
	for _, pair := range y.GetEnabledPairs(assets.AssetTypeSpot) {
		activeOrdersForPair, err := y.GetOpenOrders(y.FormatExchangeCurrency(pair, assets.AssetTypeSpot).String())
		if err != nil {
...
func (y *Yobit) GetActiveOrders(getOrdersRequest exchange.GetOrdersRequest) ([]exchange.OrderDetail, error) {
	...
	for _, currency := range getOrdersRequest.Currencies {
		resp, err := y.GetOpenOrders(y.FormatExchangeCurrency(currency, assets.AssetTypeSpot).String())
		if err != nil {
			return nil, err
		}

		for ID, order := range resp {
			symbol := pair.NewCurrencyPairDelimiter(order.Pair, y.CurrencyPairs.ConfigFormat.Delimiter)
...
func (y *Yobit) GetOrderHistory(getOrdersRequest exchange.GetOrdersRequest) ([]exchange.OrderDetail, error) {
	...
	for _, currency := range getOrdersRequest.Currencies {
		resp, err := y.GetTradeHistory(0, 10000, math.MaxInt64, getOrdersRequest.StartTicks.Unix(), getOrdersRequest.EndTicks.Unix(), "DESC", y.FormatExchangeCurrency(currency, assets.AssetTypeSpot).String())
		if err != nil {
			return nil, err
		}
