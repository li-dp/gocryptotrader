...
func (h *HUOBI) GetActiveOrders(getOrdersRequest exchange.GetOrdersRequest) ([]exchange.OrderDetail, error) {
	...
	for _, currency := range getOrdersRequest.Currencies {
		resp, err := h.GetOpenOrders(h.API.Credentials.ClientID, currency.Pair().Lower().String(), side, 500)
		if err != nil {
			return nil, err
		}
	...
	for _, order := range allOrders {
		symbol := pair.NewCurrencyPairDelimiter(order.Symbol, h.CurrencyPairs.ConfigFormat.Delimiter)
