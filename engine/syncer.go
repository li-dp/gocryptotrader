...
func (e *ExchangeCurrencyPairSyncer) get(exchangeName string, p pair.CurrencyPair, a assets.AssetType) (*CurrencyPairSyncAgent, error) {
	e.mux.Lock()
	defer e.mux.Unlock()

	for x := range e.CurrencyPairs {
		if e.CurrencyPairs[x].Exchange == exchangeName &&
			e.CurrencyPairs[x].Pair.Equal(p, true) &&
...
func (e *ExchangeCurrencyPairSyncer) exists(exchangeName string, p pair.CurrencyPair, a assets.AssetType) bool {
	e.mux.Lock()
	defer e.mux.Unlock()
...
func (e *ExchangeCurrencyPairSyncer) add(c CurrencyPairSyncAgent) {
	...
	if e.Cfg.SyncTicker {
		log.Debugf("%s: Added ticker sync item %v: using websocket: %v using REST: %v", c.Exchange, c.Pair.Pair(),
	...
	if e.Cfg.SyncOrderbook {
		log.Debugf("%s: Added orderbook sync item %v: using websocket: %v using REST: %v", c.Exchange, c.Pair.Pair(),
	...
	if e.Cfg.SyncTrades {
		log.Debugf("%s: Added trade sync item %v: using websocket: %v using REST: %v", c.Exchange, c.Pair.Pair(),
...
func (e *ExchangeCurrencyPairSyncer) remove(c CurrencyPairSyncAgent) {
	...
	for x := range e.CurrencyPairs {
		if e.CurrencyPairs[x].Exchange == c.Exchange &&
			e.CurrencyPairs[x].Pair.Equal(c.Pair, true) &&
...
func (e *ExchangeCurrencyPairSyncer) isProcessing(exchangeName string, p pair.CurrencyPair, a assets.AssetType, syncType int) bool {
	e.mux.Lock()
	defer e.mux.Unlock()
...
func (e *ExchangeCurrencyPairSyncer) setProcessing(exchangeName string, p pair.CurrencyPair, a assets.AssetType, syncType int, processing bool) {
	e.mux.Lock()
	defer e.mux.Unlock()
...
func (e *ExchangeCurrencyPairSyncer) update(exchangeName string, p pair.CurrencyPair, a assets.AssetType, syncType int, err error) {
